...
\NewDocumentCommand{\Label}{ O{unnamed} O{base} O{} m m m m }{
  ...
  \node[label #3, anchor=#2, alias=\LatestRepo-#1, name path=rec] (#1) at (c) {\tt #7};
...
\NewDocumentCommand{\Command}{ O{base} O{} O{.5} O{5} O{} m m }{
  \begin{pgfonlayer}{command-text}
    ...
    \node[command text #5, anchor=#1] (command) at (c) {\tt \$ #7};
...
% Special highlight for git keywords.
\newcommand{\gkw}[1]{%
  \tikz[baseline={(n.base)}]
    {\node[fill=Blue1](n){\vphantom{git}\tt\textbf{#1}};}%
}
