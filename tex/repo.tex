...
% [name][color][anchor]
% {hash}{offset-from-commit}{local-arrow-start}{label-text}
\NewDocumentCommand{\Label}{ O{label} O{Blue4} O{base} m m m m }{

  \node[scale=\CommitScale, label=#2, anchor=#3]
    (#1) at ($(#4) + (#5)$) {\tt #7};

  \begin{pgfonlayer}{background}
    \begin{scope}[local to=#1]
      \coordinate (arrow-start) at (#6);
    \end{scope}
    \draw[label arrow=#2] (arrow-start) -- (#4);
  \end{pgfonlayer}

}
% Basic branch label.
% [color][anchor]{hash}{offset-from-commit}{local-arrow-start}{branch-name}
\NewDocumentCommand{\Branch}{ O{Blue4} O{base} m m m m }{
  \Label[#6][#1][#2]{#3}{#4}{#5}{#6}
}
% Special HEAD label.
% [anchor]{hash}{offset-from-commit}{local-arrow-start}
\NewDocumentCommand{\Head}{ O{base} m m m }{
  \Label[HEAD][Purple4][#1]{#2}{#3}{#4}{HEAD}
  % Also highlight current commit.
  \fill[Yellow1] (#2) circle (.9*\CommitRadius);
}

% {anchor}{same-x-as}{label}
\newcommand{\LocalRepoLabel}[3]{%
  \coordinate (c) at (#2 |- Canvas.west);
  \node[scale=\NormalScale, draw=Light5, fill=Light2,
       inner sep=2, below=20 of c, anchor=#1] at (c)
    {#3};
}
% {repo-name}{accout-name}
\newcommand{\RemoteRepoLabel}[2]{%
  \coordinate (c) at (#1.west |- Canvas.north);
  \node[below=10 of c, anchor=base west, inner sep=2,
       scale=\NormalScale, Blue4, draw=Light5, fill=Light2]
    {\tt https://github.com/%
      \tikz[baseline={(n.base)},inner sep=.5]{\node[fill=Yellow1](n){#2};}%
      /Pizzas};
}
% [left|right][bend]{start}{end}
