...
% [type][hash-offset][message-offset][type]{location}{hash}{message}
\NewDocumentCommand{\Commit}{ O{} O{0} O{0} O{} m m m }{
  \coordinate[alias=\LatestRepo-#6] (#6) at (#5);
  \path[commit] (#6) circle (\CommitRadius);

  % Hash.
  \node[scale=\CommitScale, anchor=base east, hash,
        alias=\LatestRepo-#6-hash] (#6-hash)
    at ($(#6) - (\CommitRadius + \CommitMargins + #2, -\CommitBaseHeight)$)
    {\Code{#6}};

  % Message.
  \node[scale=\CommitScale, anchor=base west, Dark4,
        alias=\LatestRepo-#6-message] (#6-message)
    at ($(#6) + (\CommitRadius + \CommitMargins - #2 + #3, \CommitBaseHeight)$)
    {\Code{#7}};

  \IfSubStr{#4}{hi}{\IfSubStr{#4}{fade}{\def\op{.2}}{\def\op{1}}}{\def\op{0}}
  \HighlightSquare[.8][\op]{#6-message.south west}{#6-message.north east}
}
...
% [anchor][reponame][alignment][opacity]{location}{type/hash/message list}{labels}
\NewDocumentCommand{\Repo}{ O{center} O{repo} O{simple} O{1} m m +m }{
  ...
  \node[anchor=#1] (#2) at (repoloc) {\tikz[remember picture, baseline={(#2)},
    ...
    \foreach \type/\hash/\message [count=\i] in {#6} {\ifcsempty{hash}{}{
      ...
      \ifnumcomp{\i}{=}{1}{
        \Commit[\hash][0][\Hoff][\type]{#2}{\hash}{\message}
        ...
        \ifstrequal{#3}{mixed}{\tikzmath{\Hoff = \H;}}{}
        \Commit[\type][\H][\Hoff][\type]{\hash}{\hash}{\message}
        \IfSubStr{\type}{A}{%
...
\NewDocumentCommand{\Label}{ O{unnamed} O{base} O{} m m m m }{
  ...
  \node[label #3, anchor=#2, alias=\LatestRepo-#1, name path=rec] (#1) at (c) {\Code{#7}};
  ...
  \ifstrequal{#6}{noarrow}{}{
        \IfSubStr{#4}{base}{
          ...
          \draw[label #3, shorten >=\CommitArrowShorten mm] (cross) -- (#4);
        }
  }
...
\NewDocumentCommand{\RemoteRepoLabel}{ O{} m m m m }{{%
  ...
  \node[remote label, anchor=#2] at (c) {\Code{https://github.com/\account/\name}};
}}
