...
% [type][hash-offset][message-offset]{location}{hash}{message}
\NewDocumentCommand{\Commit}{ O{} O{0} O{0} m m m }{
  \coordinate[alias=\LatestRepo-#5] (#5) at (#4);
  \path[commit] (#5) circle (\CommitRadius);

  % Hash.
  \node[scale=\CommitScale, anchor=base east, hash,
        alias=\LatestRepo-#5-hash] (#5-hash)
    at ($(#5) - (\CommitRadius + \CommitMargins + #2, -\CommitBaseHeight)$)
    {\Code{#5}};

  % Message.
  \node[scale=\CommitScale, anchor=base west, Dark4,
        alias=\LatestRepo-#5-message] (#5-message)
    at ($(#5) + (\CommitRadius + \CommitMargins - #2 + #3, \CommitBaseHeight)$)
    {\Code{#6}};
}
...
\NewDocumentCommand{\Repo}{ O{center} O{repo} O{simple} O{1} m m +m }{
  ...
  \node[anchor=#1] (#2) at (repoloc) {\tikz[remember picture,
    ...
    \foreach \type/\hash/\message [count=\i] in {#6} {\ifcsempty{hash}{}{
      ...
      \ifnumcomp{\i}{=}{1}{
        \Commit[\hash][0][\Hoff]{#2}{\hash}{\message}
        ...
        \Commit[\type][\H][\Hoff]{\hash}{\hash}{\message}
        \IfSubStr{\type}{A}{%
...
% {color}{text}
\newcommand{\CommandHighlight}[2]{%
  \tikz[baseline={(n.base)}]
    {\node[fill=#1](n){\vphantom{git}\Code{#2}};}%
}
% Git keywords.
\newcommand{\gkw}[1]{\CommandHighlight{Blue1}{#1}}
% Just highlight one word in particular..
\newcommand{\ghi}[1]{\CommandHighlight{Light3}{#1}}
