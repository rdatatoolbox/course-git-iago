...
\tikzset{
  ...
  remote label/.style={scale=\NormalScale, above=2, sloped,
...
% If a label is given, style as a remote pointer, otherwise an illustrative flux.
% If start or end coordinates contain a comma,
% they are interpreted as global intensive coordinates,
% unless a dollar sign is also found.
% [label][bend][side]{start}{end}
\NewDocumentCommand{\RemoteArrow}{ O{} O{0} O{left} m m }{
  % Did not manage to loop over this
  % because of tikz not expanding {$(node)!.5!(node)$} prior to parsing..?
  \ifboolexpr{test {\IfSubStr{#4}{\$}}
           or test {\IfSubStr{#4}{!}}
       or not test {\IfSubStr{#4}{,}}}
    {\coordinate (s) at (#4);}
    {\IntensiveCoordinates{Canvas}{s}{#4}}
  \ifboolexpr{test {\IfSubStr{#5}{\$}}
           or test {\IfSubStr{#5}{!}}
       or not test {\IfSubStr{#5}{,}}}
    {\coordinate (e) at (#5);}
    {\IntensiveCoordinates{Canvas}{e}{#5}}
  % .. anyway.
...
\NewDocumentCommand{\Command}{ O{base} m m }{
  \begin{scope}[local to=Canvas]
    \node[scale=\CommandScale, anchor=#1,
          inner sep=2, fill=Light4, draw=Dark3, line width=2.5]
      (command) at (#2) {\tt \$ #3};
  \end{scope}
}
