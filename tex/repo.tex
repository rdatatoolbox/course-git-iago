...
\NewDocumentCommand{\Label}{ O{label} O{Blue4} O{base} O{label} m m m m }{
  ...
  \begin{pgfonlayer}{background}
    \begin{scope}[local to=#1]
      \coordinate (arrow-start) at (#7);
    \end{scope}
    \draw[label arrow=#2] (arrow-start) -- (#5);
  \end{pgfonlayer}

}
% Basic branch label.
% [color][anchor][style]
% {hash}{offset-from-commit}{local-arrow-start}{branch-name}
\NewDocumentCommand{\Branch}{ O{Blue4} O{base} O{label} m m m m }{
  \Label[#7][#1][#2][#3]{#4}{#5}{#6}{#7}
}
% Special HEAD label.
% [anchor][style]{hash}{offset-from-commit}{local-arrow-start}
\NewDocumentCommand{\Head}{ O{base} O{label} m m m }{
  \Label[HEAD][Purple4][#1][#2]{#3}{#4}{#5}{HEAD}
  % Also highlight current commit.
  \fill[Yellow1] (#3) circle (.9*\CommitRadius);
}
