...
% [padding]{node}
\NewDocumentCommand{\HighlightSquareRing}{ O{1} m }{
  \begin{pgfonlayer}{main}%
    \coordinate (pad) at (#1, #1);
    \coordinate (lo) at ($(#2.south west) - (pad)$);
    \coordinate (up) at ($(#2.north east) + (pad)$);
...
% unless they contain no comma (node name) or are a calculation with '$'.
